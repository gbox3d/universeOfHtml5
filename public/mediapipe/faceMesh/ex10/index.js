...
async function main() {
    ...
    const geometry = new THREE.TorusGeometry(10, 1, 16, 100); // (반지름, 두께, 원형 분할 수, 세그먼트 수)
    const circle_material = new THREE.MeshBasicMaterial({ color: 0xffff00, wireframe: true });
    ...
    window.addEventListener('click', () => {
        ...
        circle.position.copy(sphere.position);
    });
            ...
            if (results.faceLandmarks && results.faceLandmarks.length > 0) {
                ...
                if (results.facialTransformationMatrixes.length > 0) {
                    ...
                    if (ray.intersectPlane(plane, intersectionPoint)) {
                        ...
                        const _dist = intersectionPoint.distanceTo(circle.position);
                        // console.log(_dist);

                        const maxDistance = 10; // 최대 거리 (이 이상이면 완전 검은색)
                        const baseColor = new THREE.Color(0xffff00); // 기본 노란색
                        const darkColor = new THREE.Color(0x000000); // 완전 검은색

                        const lerpFactor = Math.min(_dist / maxDistance, 1); // 0~1 범위로 정규화
                        const newColor = baseColor.clone().lerp(darkColor, lerpFactor);

                        circle.material.color.set(newColor);
